...
def build_basic_beat_features(cleaned, rpeaks, fs):
    rpeaks = np.asarray(rpeaks, dtype=int)
    rows = []
    if len(rpeaks) < 2:
        return pd.DataFrame()
    ...
    for i, r in enumerate(rpeaks):
        if not (0 <= r < len(cleaned)):
            continue

        rr_prev_ms = np.nan
        if i >= 1:
            rr_prev_ms = (rpeaks[i] - rpeaks[i - 1]) / fs * 1000.0

        # simple local morphology features around R
        w = int(0.08 * fs)  # 80 ms window
        a = max(0, r - w)
        b = min(len(cleaned), r + w)
        seg = cleaned[a:b]

        r_amp = float(cleaned[r])
        local_std = float(np.std(seg)) if len(seg) > 3 else np.nan
        local_range = float(np.max(seg) - np.min(seg)) if len(seg) > 3 else np.nan

        # simple slope features
        slope_pre = np.nan
        slope_post = np.nan
        if r - 3 >= 0 and r + 3 < len(cleaned):
            slope_pre = float(cleaned[r] - cleaned[r - 3])
            slope_post = float(cleaned[r + 3] - cleaned[r])
...
def beat_ai_anomaly_detection(beat_df: pd.DataFrame):
    ...
    feature_priority = [
    # delineation-based (best if available)
    "qrs_ms", "qt_ms", "st_dev", "t_r_gap_ms",
    # peak-based (works on short clips)
    "rr_prev_ms", "rr_next_ms", "r_amp",
    "local_std", "local_range",
    "slope_pre", "slope_post",
    ]
    use_cols = [c for c in feature_priority if c in beat_df.columns]

    # Short ECGs often have very few beats. Allow smaller sets if enough features exist.
    min_beats = 3 if len(use_cols) >= 3 else 4
    if len(use_cols) < 2 or len(beat_df) < min_beats:
    ...
    # Keep rows that have at least 2 non-NaN feature values before filling
    valid_rows = X.notna().sum(axis=1) >= min(2, len(use_cols))
    X = X.loc[valid_rows].copy()

    if len(X) < min_beats:
        beat_df["ai_anomaly"] = 0
        beat_df["ai_score"] = np.nan
        return beat_df, {"ai_model": "skipped (not enough valid beat rows)"}
    ...
    beat_df.loc[X.index, "ai_anomaly"] = (preds == -1).astype(int)
    beat_df.loc[X.index, "ai_score"] = scores
